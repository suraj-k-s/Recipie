...
<html lang="en" >
    ...
    <body>
        <h1 align="center" id="heading">RECIPIES</h1>
        <br><br>
        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
			...
			<?php
                ...
                if (isset($_GET['aid']))
                {
                    ...
                    $select= "update tbl_recipie set recipie_status='1' where recipie_id='".$_GET['aid']."'";
                    if($Con->query($select))
                    {
                    ...
                    $mailR->SMTPAuth = true;
                    $mailR->Username = 'shinumoljohn16@gmail.com'; // Your gmail
                    $mailR->Password = 'pjdmwcfppfxffshq'; // Your gmail app password
                    $mailR->SMTPSecure = 'ssl';
                    $mailR->Port = 465;
                
                    $mailR->setFrom('shinumoljohn16@gmail.com'); // Your gmail
                
                    $user_id = $_GET["name"];
                    $recipie_title = $_GET["title"];
                    $mailR->addAddress($_GET["email"]);
  
                    $mailR->isHTML(true);
    
                    $mailR->Subject = "Accepted";
                    $mailR->Body =  "
                        your recipe :'$recipie_title' has been accepted. check your app
                    ";
                    $mailR->send();
                    ...
                    $mail->SMTPAuth = true;
                    $mail->Username = 'shinumoljohn16@gmail.com'; // Your gmail
                    $mail->Password = 'pjdmwcfppfxffshq'; // Your gmail app password
                    $mail->SMTPSecure = 'ssl';
                    $mail->Port = 465;
                
                    $mail->setFrom('shinumoljohn16@gmail.com'); // Your gmail


    $u = "select * from tbl_user where user_id != $user_id";
    $row3 = $Con->query($u);
    while($data3 = $row3->fetch_assoc())
    {
        
      $mail->addAddress($data3["user_email"]);
  
      $mail->isHTML(true);
    
      $mail->Subject = "New Recipie Addedd";
      $mail->Body =  "
          Hey User,
          struggling to think of what to make today?
          there is a new recipe : '$recipie_title' posted on anganwady recipes. come and check them right now!!!!!!!!
      ";
      $mail->send();
    }
                ...
                if (isset($_GET['rid']))
                {
                    ...
                    $select3= "update tbl_recipie set recipie_status='2' where recipie_id='".$_GET['rid']."'";
                    if($Con->query($select3))
                    {
                        ?>
                        <script>
                            alert('rejected')
				...
				$i=0;
			 $selQry = "select * from tbl_recipie r inner join tbl_user u on u.user_id=r.user_id";
			...
			?>
              ...
              <tr>
                 ...
                 <td align="center"><?php echo $data["recipie_title"]; ?></td>
                 <td align="center"><?php echo $data["user_name"]; ?></td>
                 
                 <td align="center">
                    <a href="about.php?id=<?php echo $data["recipie_id"]; ?>">About</a>
                </td>
                 <td align="center">

                    <?php 
                        if($data["recipie_status"]=="0")
                        {
                            ?>
                            <a href="recipie.php?aid=<?php echo $data["recipie_id"];?>&email=<?php echo $data["user_email"];?>&name=<?php echo $data["user_id"];?>&title=<?php echo $data["recipie_title"];?>">Accept</a> |
                    <a href="recipie.php?rid=<?php echo $data["recipie_id"];?>&email=<?php echo $data["user_email"];?>&name=<?php echo $data["user_id"];?>&title=<?php echo $data["recipie_title"];?>">Reject</a>
                            <?php
                        }
                        if($data["recipie_status"]=="1")
                        {
                            ?>
                            <a href="recipie.php?rid=<?php echo $data["recipie_id"];?>&email=<?php echo $data["user_email"];?>&name=<?php echo $data["user_id"];?>&title=<?php echo $data["recipie_title"];?>"> reject</a>
                            <?php
                        }
                        if ($data["recipie_status"]=="2")
                        {
                            ?>
                            <a href="recipie.php?aid=<?php echo $data ["recipie_id"];?>&email=<?php echo $data["user_email"];?>&name=<?php echo $data["user_id"];?>&title=<?php echo $data["recipie_title"];?>">Accept</a>
                            <?php
